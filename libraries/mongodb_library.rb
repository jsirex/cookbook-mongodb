...
class Chef::Recipe::MongoDB
  ...
  def self.check_replicaset_status(name, servers)
    ...
    connection = nil
    begin
      connection = ::Mongo::MongoReplicaSetClient.new(servers, :read => :secondary_preferred)
  ...
  def self.configure_replicaset(cluster_name, repl_name, servers)
    ...
    servers.each_with_index do |server, index|
      rs_members << {"_id" => index, "host" => server}      
    end
  ...
  def self.reconfigure_replicaset(cluster_name, repl_name, servers) 
    ...
    connection = nil
    begin
      connection = ::Mongo::MongoReplicaSetClient.new(servers, :read => :primary)
    rescue ::Mongo::ConnectionFailure => e
    ...
    if config
      ...
      new_servers.each do |new_server|
        config['members'] << {'_id' => next_index, 'host' => new_server}
        next_index +=1
      end
      ...
      begin
        ...
        result = rs_admin.command(cmd, :check_response => false)
      rescue
